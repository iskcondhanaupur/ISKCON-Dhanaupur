    ...
    <section className="section" style={{ background: 'transparent' }}>
      <div className="container" style={{ maxWidth: 600 }}>
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: 48, textAlign: 'center' }}>
          ...
          <h1 style={{ fontSize: 'clamp(28px, 5vw, 40px)', color: 'var(--maroon)', marginBottom: 12, fontFamily: ff, fontWeight: 600 }}>{c.title}</h1>
          <p style={{ fontSize: 17, color: 'var(--gold)', fontFamily: ff, marginBottom: 20 }}>{c.subtitle}</p>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} style={{ marginBottom: 48 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div style={{ background: 'var(--parchment)', border: '1px solid var(--border)', borderRadius: 12, padding: '24px 20px', textAlign: 'center' }}>
              <span className="t-label" style={{ display: 'block', marginBottom: 12, color: 'var(--gold)', fontSize: 14 }}>
                {isHi ? 'पता' : 'Address'}
              </span>
              <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="var(--maroon)" strokeWidth="1.5" style={{ margin: '0 auto 12px' }}>
                <path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/>
              </svg>
              <p style={{ fontSize: 14, color: 'var(--text)', fontFamily: ff, lineHeight: 1.7 }}>
            ...
            <div style={{ background: 'var(--parchment)', border: '1px solid var(--border)', borderRadius: 12, padding: '24px 20px', textAlign: 'center' }}>
              ...
              <p style={{ fontSize: 12, color: 'var(--muted)' }}>{isHi ? 'कॉल करें' : 'Click to Call'}</p>
            </div>
        ...
        <motion.div
          ...
          <a
            ...
            style={{
              ...
              boxShadow: '0 4px 15px color-mix(in srgb, var(--maroon) 20%, transparent)',
              transition: 'all 0.3s ease',
              cursor: 'pointer',
              border: 'none'
            }}
            onMouseEnter={(e) => {
              e.currentTarget.style.transform = 'translateY(-2px)'
              e.currentTarget.style.boxShadow = '0 6px 20px color-mix(in srgb, var(--maroon) 30%, transparent)'
            }}
            onMouseLeave={(e) => {
              e.currentTarget.style.transform = 'translateY(0)'
              e.currentTarget.style.boxShadow = '0 4px 15px color-mix(in srgb, var(--maroon) 20%, transparent)'
            }}
          >
